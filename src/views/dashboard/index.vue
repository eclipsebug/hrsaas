<template>
  <div class="dashboard-container">
    ...
    <el-row type="flex" justify="space-between">
      ...
      <el-col :span="13" style="padding-right:26px">
        ...
        <el-card class="box-card">
          ...
          <!-- 放置日历组件 -->
          <!-- 放置日历组件 -->
          <WorkCalendar :currenDate="new Date()"/>
        </el-card>
        <!-- 公告 -->
        <el-card class="box-card">
    ...
    <el-dialog title="离职申请" :visible="showDialog" @click="btnCancel">
      <el-form :rules="rules" label-width="110px" ref="ruleForm" :model="fromData">
        <el-form-item label="离职时间" prop="time1">
          <div class="block">
            <el-date-picker
                v-model="fromData.time1"
                type="datetime"
                placeholder="选择日期时间"
            >
            </el-date-picker>
...
export default {
  ...
  methods: {
    async btnOk() {
      await this.$refs.ruleForm.validate(async validate => {
        if (validate) {
          const data = { ...this.fromData, userId: this.userInfo.userId }
